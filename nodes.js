...
export async function sampleLogoForNodes() {
...
export function setupMouseRepel() {
...
function applyMouseRepel() {
    if (state.mouseRepelDisabled) return;

    state.nodes.forEach((nodeData) => {
        const dx = nodeData.logoX - state.mouseX;
        const dy = nodeData.logoY - state.mouseY;
